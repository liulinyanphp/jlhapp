...
class UtilService{
    ...
    public function moni_brower_curl($curlurl='',$referurl='')
    {
    	if($curlurl=='' || $referurl==''){
    		return 'url_error';
    	}
    	$ch = curl_init();  
		//$curlurl = "https://www.aicoin.net.cn/api/value/getDetail?key=$biname";  //目标url
		//$referurl = "https://www.aicoin.net.cn/currencies";  //模拟从哪里来
    ...
    public function requestApi($url,$requestString,$domain,$timeout = 10)
    {
    	$curl = curl_init(); // 启动一个CURL会话
	    curl_setopt($curl, CURLOPT_URL, $url);
	    curl_setopt($curl, CURLOPT_HEADER, 0);
